...
exports.doPost = async (url, opts, payload) => {
  const response = await httpUtils.post(url, opts, payload)
  try {
    const jsonData = await response.json();
    return jsonData
  } catch (error) {
    // catches the error but return undefined to allow the caller to handle the error
  }
};
...
exports.logger = content => {
  ...
  console.error(`Please post the output to ${packageJson.bugs.url} or DisTube support server. Thanks!`);
...
exports.checkForUpdates = async () => {
  ...
  try {
    ...
    const res = await httpUtils.get('https://api.github.com/repos/distubejs/ytsr/contents/package.json', {
